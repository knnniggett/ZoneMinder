...
?>
            <tr<?php if ($event->Archived()) echo ' class="archived"' ?>>
              <td data-checkbox="true" name="eids[]" value="<?php echo $event->Id() ?>"></td>            
              <td><a href="?view=event&amp;eid=<?php echo $event->Id().$filterQuery.$sortQuery.'&amp;page=1">'.$event->Id().($event->Archived()?'*':'') ?></a></td>
